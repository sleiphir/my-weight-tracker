...
<table class="text-center border-spacing-x-4 border-spacing-y-3 border-separate">
  ...
  <tbody>
    {#each weightList as data, index}
    <tr>
      <td>
        <input data-index={index} data-datetime={data.date} type="number" step="0.1" id="weight-input" aria-describedby="Weight input" value={data.weight} on:change={save} class="border text-sm rounded-lg block w-full p-2.5  bg-zinc-700 border-zinc-600 placeholder-zinc-400 text-white focus:ring-zinc-500 focus:border-zinc-500" placeholder="Weight (kg)">
      </td>
      <td>
